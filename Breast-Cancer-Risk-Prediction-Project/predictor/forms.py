...
class FactorsForm(FlaskForm):
    ...
    first_degree_relative = SelectField(
        "First degree relative", choices=[("no", "No"), ("yes", "Yes")]
    )
    brca1 = SelectField("BRCA1", choices=[("no", "No"), ("yes", "Yes")])
    brca2 = SelectField("BRCA2", choices=[("no", "No"), ("yes", "Yes")])
    menarche_age = IntegerField(
        "Menarche age", validators=[Optional(), NumberRange(min=6, max=30)]
    )
    menopause_age = IntegerField(
        "Menopause age", validators=[Optional(), NumberRange(min=30, max=70)]
    )
    ...
    alcohol_units = IntegerField(
        "Alcohol units per week", validators=[Optional(), NumberRange(min=0, max=100)]
    )
    smoking = SelectField("Smoking status", choices=SMOKING_CHOICES, default="never")
    activity_hours = IntegerField(
        "Activity hours per week", validators=[Optional(), NumberRange(min=0, max=168)]
    )
    submit = SubmitField("Predict")
